...
const UpdateSongForm = ({ navigation, route }) => {
    ...
    useEffect(() => {
        if (Songs.length > 0) {
            const firstSong = Songs[0]; // Assuming you want to set values based on the first song
            //console.log(firstSong.song)
            setValues({
                name: firstSong.songName,
                author: firstSong.songWrite,
                date: firstSong.SongDate,
                song: firstSong.song,
                key: firstSong.songKey,
            });
        }
    }, [Songs]);
    ...
    function addSong(title, writer, date, song, key) {
        ...
        updateDoc(doc(db, 'Songs', song_id), newData)
            .then(() => {
                navigation.navigate('UpdateSong')
                Alert.alert("Indirimbo Yawe Yahindutse Neza");
    ...
    const [btnColor, setBtnColor] = useState('rgb(37, 100, 40)');
        ...
        <SafeAreaView style={styles.mainBg} >
            <ScrollView>
                ...
                <View style={styles.form}>
                    <Text style={styles.title}>Hindura Indirimbo</Text>
